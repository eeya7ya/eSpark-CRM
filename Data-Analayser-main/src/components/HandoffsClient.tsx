...
const PRIORITY_STYLE: Record<Handoff["priority"], string> = {
  urgent: "bg-espark-primary/10 text-espark-primary border-espark-primary/30",
  high: "bg-amber-50 text-amber-800 border-amber-300",
  normal: "bg-espark-soft text-espark-ink/60 border-espark-border",
  low: "bg-espark-soft text-espark-ink/45 border-espark-border",
};
...
const STATUS_STYLE: Record<Handoff["status"], string> = {
  ...
  cancelled: "bg-espark-soft text-espark-ink/50",
};
    ...
    <div className="space-y-3">
      ...
      {handoffs.length === 0 ? (
        <div className="rounded-2xl border border-espark-border bg-espark-surface p-8 text-center">
          <p className="text-sm text-espark-ink/60">No project handoffs yet.</p>
          <p className="mt-0.5 text-xs text-espark-ink/40">
...
function HandoffCard({
  ...
  return (
    <li className="rounded-2xl border border-espark-border bg-espark-surface p-4 shadow-es-soft">
      <div className="flex flex-wrap items-start justify-between gap-2">
        ...
        <button
          ...
          className="inline-flex items-center gap-1 rounded-full bg-espark-soft px-2 py-0.5 text-[11px] font-semibold text-espark-ink/60 hover:bg-espark-soft/70 disabled:cursor-default disabled:opacity-60 transition-colors"
        >
      ...
      {showBoq && h.boq_snapshot && h.boq_snapshot.length > 0 && (
        <div className="mt-3 overflow-hidden rounded-xl border border-espark-border">
          <table className="w-full text-left text-[11px]">
            <thead className="bg-espark-soft/60 text-espark-ink/55">
              <tr>
            ...
            <tbody>
              {h.boq_snapshot.map((line, i) => {
                if (line.section) {
                  return (
                    <tr key={i} className="bg-espark-soft/30">
                      <td colSpan={4} className="px-2 py-1 font-semibold text-espark-ink/70">
                ...
                return (
                  <tr key={i} className="border-t border-espark-border/60 align-top">
                    <td className="px-2 py-1 text-espark-ink/45">{i + 1}</td>
                    <td className="px-2 py-1 font-medium text-espark-ink/80">{item}</td>
                    <td className="px-2 py-1 text-espark-ink/60">{line.description || "—"}</td>
                    <td className="px-2 py-1 text-right text-espark-ink/70">{line.quantity ?? "—"}</td>
                  </tr>
                );
              })}
            </tbody>
      ...
      <div className="mt-3 grid gap-2 text-xs text-espark-ink/70 sm:grid-cols-2">
        {(h.contact_name || h.contact_phones) && (
          <div className="flex items-start gap-1.5">
            <User className="mt-0.5 h-3.5 w-3.5 shrink-0 text-espark-ink/40" />
            <span>
              {h.contact_name}
              {h.contact_phones && (
                <span className="flex items-center gap-1 text-espark-ink/55">
        ...
        {(h.site_address || (h.location_lat != null && h.location_lng != null)) && (
          <div className="flex items-start gap-1.5">
            <MapPin className="mt-0.5 h-3.5 w-3.5 shrink-0 text-espark-ink/40" />
            <span>
              {h.site_address}
              {h.location_lat != null && h.location_lng != null && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="block font-semibold text-espark-primary hover:underline"
                >
      ...
      {h.notes && (
        <p className="mt-2 rounded-lg bg-espark-soft/50 px-3 py-2 text-xs text-espark-ink/70">
          {h.notes}
        </p>
      )}

      {assigned && (
        <p className="mt-2 text-xs text-espark-ink/55">
          Assigned to <span className="font-semibold text-espark-ink/80">{assigned}</span>
          {h.assigned_at && <> · {new Date(h.assigned_at).toLocaleDateString()}</>}
        </p>
      )}

      {(canAssign || h.can_complete) && (
        <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-espark-border/60 pt-3">
          {canAssign && (
            <div className="w-full space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                <Select
                  ...
                  disabled={busy}
                  className="rounded border border-espark-border bg-espark-surface px-2 py-1.5 text-xs"
                >
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                <input
                  ...
                  className="rounded border border-espark-border bg-espark-surface px-2 py-1.5 text-xs sm:col-span-2"
                />
                <label className="flex items-center gap-1.5 text-[11px] text-espark-ink/55">
                  Start
                  <input
                    type="date"
                    value={startDate}
                    ...
                    disabled={busy}
                    className="flex-1 rounded border border-espark-border bg-espark-surface px-2 py-1.5 text-xs"
                  />
                </label>
                <label className="flex items-center gap-1.5 text-[11px] text-espark-ink/55">
                  End
              ...
              <button
                ...
                className="rounded bg-espark-ink px-3 py-1.5 text-xs font-semibold text-espark-on-ink hover:bg-espark-primary disabled:opacity-50 transition-colors"
              >
